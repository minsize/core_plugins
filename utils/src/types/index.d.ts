...
// Добавляем новый глобальный метод `lang` для работы с текстовыми строками
declare global {
  /**
   * @example
  ...
  const decWord: (n: number, words: string[]) => string
   ...
   */
  function alignTo(num: number, by: number): number
  ...
  function random(min: number, max: number, seed?: number): number
   ...
   */
  const generateUniqueKey: <VALUE extends unknown>(obj: VALUE) => string

  const unlink: <VALUE extends unknown>(value: VALUE) => VALUE
   ...
   */
  const chunks: <T>(count: number, array: T[]) => T[][]
